...
1. **Load** -- `door.loader.load_stream` reads the CSV as `utf-8-sig`, rejects the file up
   front if any of `Datetime`, `Close command`, `Open command` or `Door leaf position` is
   missing, and builds one `DoorSample` per row. Each sample keeps the original timestamp
...
2. **Segment** -- `door.segmentation.detect_intervals` cuts the stream wherever the asserted
   ...
   one oversized interval. Intervals are contiguous -- each runs to the sample before the
   next start -- anything under 0.5 s is discarded, and each is tagged `Open` or `Close`.
...
4. **Classify** -- `door.pipeline.DoorModel` fits a class-balanced Extra Trees (200 trees,
   ...
   intervals matching nothing are left out rather than mislabelled. If scikit-learn is not
   installed, or fewer than two distinct classes or four labelled rows survive, the model
