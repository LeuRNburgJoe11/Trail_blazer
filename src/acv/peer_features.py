...
def get_simple_peer_median(cars_data: Dict[str, pd.DataFrame], signal: str) -> pd.DataFrame:
    ...
    combined = pd.DataFrame({car_id: cars_data[car_id].get(signal, pd.Series(dtype=float)) for car_id in car_ids})
...
def get_context_matched_peer_median(cars_data: Dict[str, pd.DataFrame], signal: str) -> pd.DataFrame:
    ...
    combined_signal = pd.DataFrame({c: cars_data[c].get(signal, pd.Series(dtype=float)) for c in car_ids})
    combined_mode = pd.DataFrame({c: cars_data[c].get("ACV Running Mode", pd.Series(dtype=str)) for c in car_ids})
    combined_load = pd.DataFrame({c: cars_data[c].get("Load Halved", pd.Series(dtype=float)) for c in car_ids})
    combined_sp = pd.DataFrame({c: cars_data[c].get("Cooling Setpoint", pd.Series(dtype=float)) for c in car_ids})
